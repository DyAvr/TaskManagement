...
using HomeworkApp.Bll.Entities;
...
public class TaskService : ITaskService
{
        ...
        CancellationToken token)
    {
        ...
        var cachedComments = await _distributedCache.GetStringAsync(cacheKey, token);
        if (!string.IsNullOrEmpty(cachedComments))
        {
            return JsonSerializer.Deserialize<TaskMessage[]>(cachedComments) ?? new TaskMessage[] {};
        }
        ...
        var taskMessages = taskComments
            .OrderByDescending(c => c.At)
            .Select(c => new TaskMessage
            {
                TaskId = c.TaskId,
                Comment = c.Message,
        ...
        {
            var latestTaskMessages = taskMessages.Take(5).ToArray();
            var serializedComments = JsonSerializer.Serialize(latestTaskMessages);
            await _distributedCache.SetStringAsync(
                cacheKey, 
                serializedComments, 
                new DistributedCacheEntryOptions 
